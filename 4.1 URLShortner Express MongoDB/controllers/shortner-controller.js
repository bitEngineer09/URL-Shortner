...
export const getShortnerPage = async (req, res) => {
    try {
        ...
        // Hum database se links find kar ke use array bana de re hai
        const linksArr = await loadLinks();

        // Create an object where keys = shortCode, values = original URLs
        const links = {};
        linksArr.forEach(link => {
            links[link.shortCode] = link.url;
        });


        // Render 'index.ejs' and pass the links + host name to the view
        // req.get('host') gets the host name from the incoming HTTP request headers.
        // Example: If the user visits http://localhost:3000, req.get('host') will return localhost:3000.
